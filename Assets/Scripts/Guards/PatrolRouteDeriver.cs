...
namespace Guards
{
    ...
    {
        public static List<Vector2Int> Derive(
            string homeRoomId, RoomGraph graph, IReadOnlyDictionary<string, RoomRect> rects)
        {
        ...
        private static void AddRoomCentre(
            List<Vector2Int> route, IReadOnlyDictionary<string, RoomRect> rects, string roomId)
        {
            if (rects.TryGetValue(roomId, out var rect))
                route.Add(new Vector2Int(rect.CenterX, rect.CenterY));
        }
