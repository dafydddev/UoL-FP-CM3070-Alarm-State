...
namespace Spawners
{
    // Spawns one guard at every guard-post room, handing each its patrol route
    // derived from the room graph. Guard-post placement (and how often it happens)
    // is decided upstream by the room graph generator via the difficulty profile.
